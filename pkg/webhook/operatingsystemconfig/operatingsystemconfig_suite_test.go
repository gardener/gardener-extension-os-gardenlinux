...
var _ = Describe("Ensurer", func() {
	...
	It("Should replace the containerd cgroup driver in the CRIConfiguration", func() {
		...
		Expect(ensurer.EnsureContainerdConfig(ctx, nil, c, nil)).To(Succeed())
		Expect(c.CRICgroupDriver).To(Equal(extensionsv1alpha1.CRICgroupDriverSystemd))
	})
})
...
var _ = Describe("Mutator", func() {
	...
	Describe("#MutateGardenLinuxOSC", func() {
		...
		It("should not mutate an OperatingSystemConfig not for Garden Linux", func() {
			...
			Expect(osc.Spec.CRIConfig.CRICgroupDriver).NotTo(Equal(extensionsv1alpha1.CRICgroupDriverSystemd))
		})
	})
})
